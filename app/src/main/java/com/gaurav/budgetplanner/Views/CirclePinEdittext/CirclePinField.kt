...
class CirclePinField: PinField{
    ...
    var circleRadiusDp = Utils.dpToPx1(10f)
        set(value){
            field = value
            invalidate()
        }
    ...
    private fun initParams(attr: AttributeSet){
        ...
        try {
            ...
            if(distanceInBetween == DEFAULT_DISTANCE_IN_BETWEEN) distanceInBetween = Utils.dpToPx1(30f)
        } finally {
            a.recycle()
        }
    }
    ...
    private fun getThickness(): Int{
        return Math.round(if(!highlightNoFields())highLightThickness else lineThickness).toInt()
    }
