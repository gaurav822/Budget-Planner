...
class CountryListAdapter():RecyclerView.Adapter<CountryListAdapter.ViewHolder>() {
    ...
    var onEachItemClick:((CurrencyModel) -> Unit)?=null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            onEachItemClick?.invoke(
                CurrencyModel(
                currencyCode = allItem[position].key,
                countryFlag = "https://flagsapi.com/${allItem[position].value["isoAlpha2"]}/flat/64.png",
                    currencySymbol = allItem[position].value["symbol"]!!,
            )
            )
